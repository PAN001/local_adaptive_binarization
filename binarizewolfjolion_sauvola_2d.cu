...
__global__ void NiblackSauvolaWolfJolionCuda(unsigned char* input, unsigned char* output, int winx, int winy, double k, int img_width, int img_height, int width_step, int rows_per_thread) {
    ...
    int col_start_idx = blockIdx.x * blockDim.x + threadIdx.x; // row index
    ...
    if(col_start_idx > img_width - winx)
        return;

    int row_idx = row_start_idx;
    int col_idx = col_start_idx;
    ...
    for(int wy=0 ; wy<winy; wy++) {
        for(int wx=0 ; wx<winx; wx++) {
            ...
            foo = input[(row_idx-wyh+wy) * width_step + (col_idx-wxh+wx)];
            sum += foo;
            sum_sq += foo*foo;
        }
    }
    ...
    if (row_idx==y_firstth)
        for (int u=0; u<y_firstth; ++u)
            set_color(input, output, u, col_idx+wxh, th, width_step);
    ...
    for (int i=x_lastth; i<img_width; ++i)
        set_color(input, output, row_idx, col_idx, th, width_step);

    // RIGHT-UPPER CORNER
    if (row_idx==y_firstth)
        for (int u=0; u<y_firstth; ++u)
            for (int i=x_lastth; i<img_width; ++i)
                set_color(input, output, u, col_idx, th, width_step);
...
void NiblackSauvolaWolfJolionWrapper(Mat input, Mat output, int winx, int winy, double k) {
    ...
    int r_total_cnt = y_lastth - y_firstth + 1;
    int c_total_cnt = input.cols-winx;
